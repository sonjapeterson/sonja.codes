...
That’s exactly what makes fixing flaky tests so challenging. Trying random fixes and testing them by running the test over and over to see if it fails doesn’t work well and is a very slow feedback loop.
...
Another thing I learned from this experience is that even just a few flaky tests can really slow down your team. When a test fails without actually signaling something wrong with the test suite, you not only have to re-run all the tests before you’re ready to deploy your code, which slows down the deployment process, you also lose some trust in your test suite, and eventually might even start ignoring real failures because you assume they’re just flakes.
...
* **Try replicating the failure with same set of tests in the same order**:  If you have your test suite configured to run in a random order, you can set the SEED value in the test command to recreate the order.
...
* **Set a target for your master branch pass rate and track it week over week**: for example, your goal might be to have builds on your master branch pass 90% of the time. This helps you keep an eye on whether you're progressing towards that goal or not, and course correct if your efforts aren't working.
