...
image: "https://sonja.codes/assets/images/posts/flaky_tests.png"
---
...
Back in my first year as a software engineer, I’d worked really hard on building out a complicated form. I wrote lots of unit and feature tests to make sure I didn’t miss any edge cases. Everything was working great, and we shipped it! But then, a few days later, we had a problem.

A red build on our main branch. The failing test was one of the feature tests for my form. Huh?? It had been working fine for days! And on the next test run, it went right back to passing.
...
The only problem was: I had never fixed a flaky test before and had no concept of why the same test, running against the same code, could pass or fail on different runs.

So I did what I often did when debugging problems I didn’t really understand at all… I attempted to use trial and error. I tried to fix the test by making a random change without really understanding how it would help.

That kind of approach sometimes works with normal bugs - you can start by using trial and error, quickly ruling out various potential solutions, and eventually you'll hit on the right one and be able to reason backwards about why it worked. But it didn't work at all with this flaky test, because I could never be sure if my random fixes were correct. I'd run it with the fix 50 times without seeing a failure and think that I solved it, but the next day we'd see it fail agian.

That’s exactly what makes fixing flaky tests so challenging. Trying random fixes and testing them by running the test over and over to see if it fails is a slow and unreliable feedback loop.
...
Another thing I learned from this experience is that even just a few flaky tests can really slow down your team. When a test fails without actually signaling something wrong with the test suite, you not only have to re-run all the tests before you’re ready to deploy your code, which slows down the deployment process, but you also lose some trust in your test suite. Eventually, you might even start ignoring real failures because you assume they’re just flakes.

So it’s very important to learn how to fix flaky tests efficiently, and better yet avoid writing them in the first place.
...
One of the key differences was that, instead of trying a fix that I didn't fully understand and then reasoning out why it did or didn't work after, I made sure I had a strong theory explaining exactly what was happening and why my fix would work before implementing it. I also didn't necessarily focus on replicating the flaky test until I had already started to build a theory of what was going on, since some types of flaky tests will be nearly impossible to replicate reliably. It required a mental shift from how I approached other kinds of bugs, especially earlier in my career, but it worked way better.
...
Thinking about it that way made fixing flaky tests more enjoyable. It actually became a fun challenge for me instead of a frustrating and tedious problem.
...
- Time of day each failure occurred
- How often the test is failing
- Which tests were run before, in what order

One way to collect this information is to add some code to your test suite that sends the output from any failures that occur on your main branch (or whichever branch you wouldn't expect to see failures on, since tests need to pass before merging into it) to a bug tracker with all the metadata you’d need, including a link to the CI build where they failed. I’ve had success doing this with [Rollbar](https://rollbar.com/) in the past, but I’m sure other bug trackers would work as well.

When doing that, it’s important to make sure the failures for the same test are grouped together - it might take a little extra configuration to get this to work, but it’s really helpful so that you can cross reference between different occurrences of the same failure and figure out what they might have in common.
...
**Definition**: a test in which some code runs asynchronously, which means that events in the test can happen in more than one order

Async code is, in my experience, one of the biggest sources of flaky tests, especially when testing Rails apps.

The most common way this comes up is in end to end tests - for web apps, these are generally tests that spin up your app's server and interact with it via a headless browser, to simulate the actual interactions a user would have with an app. Most Rails apps use [Capybara](https://github.com/teamcapybara/capybara), either through Rails’ built in system tests or through RSpec feature tests, to write these kinds of tests.
...
But, that happy path isn't the only order events in the test can occur in. Since we’re not explicitly waiting on anything that would signal that the request to create the post has finished, our test code can move right along from #1, triggering the click, to #4, checking the database, before steps 2, 3 and 4 have finished. As a result, we check the database too soon and the post isn't present yet.

The fix here is relatively simple - we just need to make sure that we wait until the request has finished before we try to check the database. We can do this by adding one of Capybara’s [waiting finders](https://github.com/teamcapybara/capybara#asynchronous-javascript-ajax-and-friends), like `have_content`, which will look for something on the page and retry until it shows up.
...
This results in basically a "doubleclick" effect: rather than the two separate page reloads we want, so the page just reloads once with the books in the ascending alphabetical order, and we never get to that reversed alphabetical order state.

The solution here would be to wait on something besides the fact that the books are in the correct order to make sure that the first request has completed before we try to trigger the second one. Here's how the fixed test might look:
...
- **Don’t use `sleep` - wait for something specific**: using `sleep` is unreliable, since your code can run at different speeds due to many different factors, and it's also often slower than waiting for the specific event that you need to wait for.
...
**Definition**: Order dependent tests are tests that can pass or fail depending on which tests ran before them.

Usually, this is caused by some sort of state leaking between tests - so when the state another test creates is present (or missing) it can cause the flaky test to fail.
...
- **Truncation or deletion**
  - Because of the limitations of transactional cleanup, many Rails test suites use the [DatabaseCleaner](https://github.com/DatabaseCleaner/DatabaseCleaner) gem to clean with either truncation - truncating all tables touched in the test - or deletion, which uses DELETE FROM to delete all records created in a test.
  ...
  - If you use DatabaseCleaner, you should make sure you’re running it in an `append_after(:each)` block, not an `after(:each)` block ([see docs](https://github.com/DatabaseCleaner/database_cleaner#rspec-with-capybara-example)) - this ensures it runs after Capybara’s cleanup, which includes waiting for any lingering ajax requests that could affect the state of the database and need to be cleaned up.

#### Example

Let's say we're using DatabaseCleaner with the truncation strategy:

```ruby
DatabaseCleaner.strategy = :truncation
```

But we notice that this is slow, so someone comes along to optimize the test suite. They notice we're creating book genres in almost all of the tests, so they decide to do that before the entire suite runs using a `before(:all)` block, and then exclude those from being cleaned up with DatabaseCleaner, with the built in option for that:

```ruby
DatabaseCleaner.strategy = :truncation, { except: %w[book_genres] }
```

This saves us some time, but introduces a gap in our cleaning. If we make any kind of modification to book genres, that update won't be undone, because we'll never truncate those tables. And this could potentially affect later tests, and show up as an order dependent flake.
...
The solution here would be to either stop excluding book genres from the tables to be truncated, or create a new book genre specifically for this test & delete it afterwards.
...
- **Try replicating the failure by running the same set of tests in the same order**: I mentioned that for many flaky tests, it doesn't make sense to try to replicate them - but if you suspect the test is order dependent, that's often an exception to this rule. If you have your test suite configured to run in a random order, you can set the SEED value in the test command to recreate the order. If multiple test files were being run when the failure occurred, make sure to run all of them when you attempt to replicate.
- **Cross reference each failed occurrence and see if the same tests ran before the failure**: RSpec has a built in [bisect](https://relishapp.com/rspec/rspec-core/docs/command-line/bisect) tool you can use to help narrow down a set of tests to the ones producing the dependency - however you may find that it runs prohibitively slowly, so sometimes it’s better to look at it manually. Look at each time it failed on CI, cross-reference which test files were run before the failure each time, and see where the overlap is.
...
def set_default_due_date
  self.due_date = Date.tomorrow.end_of_day if due_date.nil?
end
```
...
Here’s an example of a test data factory that uses FactoryBot to create an event:
...
- Try to see if you can reliably replicate failure with the same `--seed` option added to the command, since this will ensure the same random values are generated.
  - In minitest, this should work out of the box because it uses `Kernel.srand` as its default seed value.
  - In RSpec, make sure you’ve set `Kernel.srand(config.seed)` in your spec_helper.

#### Preventing randomness-based flakes

- Remove randomness from your tests & instead explicitly test boundaries & edge cases.
- Avoid using gems like [Faker](https://github.com/stympy/faker) to generate data in tests: they're great for generating realistic dev data, but in your tests it's more important to have reliable behavior & a clear sense of what's being tested than realistically random data.
...
- **Do try to find a way to reliably replicate failures to prove your theory**: once you have an idea of how a flake could be happening, it's ok at that point to use some experimentation to prove to yourself that it's the right fix. The one category I don't recommend trying this with is async flakes - it's typically too difficult to control the order and speed at which events in the test are occurring, so with those it's best to skip attempting to replicate even if you think you know what's happening.
...
If you're writing automated tests, you have to accept that at some point, you're going to deal with flaky ones. You can't just delete any test that starts to flake because you'll end up making significant compromises in the test coverage you have for your app. Also, learning to fix and avoid flaky tests is a skill you can develop over time - one that's worth investing in, even if that means spending two days fixing one the first time you have to do it, instead of just deleting it.
...
- **Make sure someone is assigned to each active flake & this type of work is spread across the whole team**: Make sure you have a specific person assigned to each active flake. That person is in charge of looking for a fix, temporarily disabling the test in prod while they work on it if it's frequently flaking, reaching out to others for help if they're stuck, and so on. It's important to make sure this responsibility is spread out among your team - don't let just one person end up being the flake expert and everyone else ignores them. If you're already sending your flake failures to a bug tracker as I suggested earlier, you can use that as a place to assign who's working on them.
- **Set a target for your main branch pass rate and track it week over week**: for example, your goal might be to have builds on your main branch pass 90% of the time. This helps you keep an eye on whether you're progressing towards that goal or not, and course correct if your efforts aren't working.

## Conclusion

To wrap this all up: if you remember just one thing from this post, I hope it's that flaky tests don't just have to be just an annoying and frustrating problem, or something you try to ignore. But they do require a different approach from your typical bug: instead of using trial and error, you'll need to gather evidence, determine what category of flaky test it fits into, and form a theory of how it's happening before attempting a fix. Fixing flaky tests can be a great opportunity to gain a deeper understanding of your tools and your code, and to pretend you're a detective for a little while. Hopefully this post has made it easier to do that.
